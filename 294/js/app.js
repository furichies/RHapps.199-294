/**
 * RH294 Student Guide - SPA Application
...
(function () {
  ...
  let activeTab = 'all';
  let readSections = JSON.parse(localStorage.getItem('rh294_read') || '{}');
    ...
    contentView.innerHTML = `
      <div class="content-header">
        <div class="content-breadcrumb">
          RH294 <span>›</span> Capítulo ${ch.number}
        </div>
        <h1>${ch.title}</h1>
  ...
  async function loadSections(sections) {
    ...
    localStorage.setItem('rh294_read', JSON.stringify(readSections));
    container.innerHTML = htmlParts.join('');
    renderSidebar(); // update progress
  }
  ...
  function formatContent(text) {
    ...
    // Detect code blocks (indented lines or lines with $ prompts)
    let inCode = false;
    let codeLines = [];
    let result = [];
  ...
  function processLine(line) {
    ...
    // Headers based on content patterns
    ...
    if (/^(Nota|Importante|Advertencia|Sugerencia|NOTA|IMPORTANTE)[:.]/.test(line.trim())) {
  ...
  function toggleTheme() {
    ...
    localStorage.setItem('rh294_theme', next);
    updateThemeIcon(next);
  }

  function loadTheme() {
    const saved = localStorage.getItem('rh294_theme') || 'dark';
